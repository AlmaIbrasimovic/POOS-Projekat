...
def detectPlatesInScene(imgOriginalScene):
    ...
    if Main.showSteps == True:
        ...
        cv2.imshow("1b", imgThreshScene)
            ...
            # pronaci sve moguce charove na slici,
    ...
    for listOfMatchingChars in listOfListsOfMatchingCharsInScene:                   
        ...
        if possiblePlate.imgPlate is not None:                          # ukoliko je tablica pronadena
            listOfPossiblePlates.append(possiblePlate)                  # dodati  u listu mogucih tablica
        # end if
    # end for
...
def findPossibleCharsInScene(imgThresh):
    ...
    contours, npaHierarchy = cv2.findContours(imgThreshCopy, cv2.RETR_LIST, cv2.CHAIN_APPROX_SIMPLE)   # pronaci sve konture
...
def extractPlate(imgOriginal, listOfMatchingChars):
    ...
    listOfMatchingChars.sort(key = lambda matchingChar: matchingChar.intCenterX)       
            ...
            # izracunati sredisnju tacku na tablici
    fltPlateCenterX = (listOfMatchingChars[0].intCenterX + listOfMatchingChars[len(listOfMatchingChars) - 1].intCenterX) / 2.0
    fltPlateCenterY = (listOfMatchingChars[0].intCenterY + listOfMatchingChars[len(listOfMatchingChars) - 1].intCenterY) / 2.0

    ptPlateCenter = fltPlateCenterX, fltPlateCenterY

            # sirina i visina tablice
